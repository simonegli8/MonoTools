...
namespace MonoTools.Debugger {
	...
	public class Pdb2MdbGenerator {
		...
		public void GeneratePdb2Mdb(string directoryName, Action<string> Log = null) {
			...
			Parallel.ForEach(files, file => {
				try {
					...
					if (File.Exists(pdbFile) && (!File.Exists(mdbFile) || File.GetLastWriteTimeUtc(pdbFile) >= File.GetLastWriteTimeUtc(mdbFile))) {
						Log?.Invoke("Generate mdb for: " + Path.GetFileName(file));
			...
			Log?.Invoke("Transformed Debuginformation pdb2mdb");
		}
