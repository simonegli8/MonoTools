...
namespace MonoTools.Debugger {
	...
	public class Launcher: ILauncher {
		...
		public virtual string XspCommand => $"{(Web.Framework == Frameworks.Net4 ? "xsp4" : "xsp2")} {MonoOptions}  --root={Map(RemotePath)} --verbose --printlog --loglevels={Web.LogLevels} --applications=/:. --nonstop --port={Web.XspPort}{SslOptions}";
		...
		public virtual void Build() {
			...
			if (Task.NetFXBuild) {
				...
				mdbs.GeneratePdb2Mdb(SourcePath, Log);
			}
		...
		public virtual void Start() {
			...
			if (IsProgram) {
				...
				Host.ChangeDir(RemotePath);
				Host.Run(XspCommand).Wait(60000);

				OpenBrowser(Web.OpenUrl);
			}
		}
		...
		public void Dispose() { Host?.Dispose(); Local?.Dispose(); }
