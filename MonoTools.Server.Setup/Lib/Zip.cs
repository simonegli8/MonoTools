...
namespace Silversite.Services {
	...
	public class Zip {
		...
		public static void Create(string paths, string file, Func<string, bool> filter = null, Action<ProgressArgs> progressHandler = null) {
			using (var zip = ZipStorer.Create(file, "Generated by MSPControl")) {
				zip.EncodeUTF8 = true;
				if (progressHandler !=null) zip.Progress += progressHandler;
				zip.AddAll(paths, filter);
			}
		}

		public static void Create(string paths, IEnumerable<string> files, string file, Action<ProgressArgs> progressHandler = null) {
			using (var zip = ZipStorer.Create(file, "Generated by MSPControl")) {
				zip.EncodeUTF8 = true;
				if (progressHandler != null) zip.Progress += progressHandler;
		...
		public static void Extract(string file, string to, Func<string, string> filter = null, Action<ProgressArgs> progressHandler = null) {
			using (var zip = ZipStorer.Open(file, System.IO.FileAccess.Read)) {
				if (progressHandler != null) zip.Progress += progressHandler;
				zip.ExtractAll(to, filter);
			}
		}
		public static void Extract(Stream file, string to, Func<string, string> filter = null, Action<ProgressArgs> progressHandler = null) {
			using (var zip = ZipStorer.Open(file, System.IO.FileAccess.Read)) {
				if (progressHandler != null) zip.Progress += progressHandler;
				zip.ExtractAll(to, filter);
			}
...
namespace System.IO.Compression {
	...
	using Silversite.Services;
	 ...
	 public class ZipStorer : IDisposable
	 {
		...
		public void ExtractAll(string to, Func<string, string> filter = null) {
			...
			foreach (var entry in ReadCentralDir()) {
				...
				if (filter != null) path = filter(path);
				if (path == null) continue;

				var rootedPath = Path.IsPathRooted(path);
				path = path.Replace('/', Path.DirectorySeparatorChar);

				if (!rootedPath) path = Path.Combine(to, path);

				ExtractFile(entry, Path.Combine(to, entry.FilenameInZip.Replace('\\', '/')));
				n++;
