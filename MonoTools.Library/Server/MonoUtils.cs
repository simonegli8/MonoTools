...
namespace MonoTools.Library {
	internal static class MonoUtils {
		...
		public static string GetMonoXsp(Frameworks framework = Frameworks.Net4) {
			var p = Environment.OSVersion.Platform;
			if (p == PlatformID.Unix || p == PlatformID.MacOSX || p == PlatFormIDUnixUnderNET1) {
				return framework == Frameworks.Net4 ? "xsp4" : "xsp2";
			}

			return Path.Combine(GetMonoRootPathWindows(), framework == Frameworks.Net4 ? @"bin\xsp4.bat" : @"bin\xsp2.bat");
		}
